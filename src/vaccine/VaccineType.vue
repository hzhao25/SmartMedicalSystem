  ...
  <div style="width: 100%">
    ...
    <div class="botoom_div">
      ...
      >
        ...
        <!-- 新增表单弹窗 -->
    <el-dialog :visible.sync="addFormVisible" title="添加疫苗种类信息">
      <el-form :model="addPosts" ref="addForm">
        <el-form-item label="疫苗种类名">
          <el-input v-model="addPosts.name"></el-input>
        </el-form-item>
        <el-form-item label="疫苗种类描述">
    ...
    <!-- 修改表单弹窗 -->
    <el-dialog :visible.sync="updateFormVisible" title="修改疫苗种类信息">
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      request
        .post("/vaccineType/deleteVaccineType", { id: row.id })
        .then((res) => {
          if (res.flag) {
            this.$message.success("删除疫苗种类信息成功");
            this.selectPage(); //重新查询数据
          } else {
            this.$message.error("删除疫苗种类信息失败");
          }
        })
        .catch((error) => {
          this.$message.error("删除疫苗种类信息出错：" + error.message);
        });
    },
    ...
    batch_delete() {
      ...
      console.log(data)
      request
        .post("/vaccineType/deleteBatchVaccineType", data, {
          jsonRequest: true,
        })
        .then((res) => {
          if (res.flag) {
            this.$message.success("批量删除疫苗种类信息成功");
            this.selectPage();
          } else {
            this.$message.error("批量删除疫苗种类信息失败");
          }
        })
        .catch((error) => {
          this.$message.error("批量删除疫苗种类信息出错：" + error.message);
        });
    },
    ...
    updateRecord() {
      request
        .post("/vaccineType/updateVaccineType", this.updatePosts)
        .then((res) => {
          if (res.flag) {
            this.$message.success("修改疫苗种类信息成功");
            this.updateFormVisible = false;
            this.selectPage(); // 重新查询数据
          } else {
            this.$message.error("修改疫苗种类信息失败");
          }
        })
        .catch((error) => {
          this.$message.error("修改疫苗种类信息出错：" + error.message);
        });
    },
    // 显示新增表单弹窗
    showAddForm() {
    ...
    addRecord() {
        request
          .post("/vaccineType/managerAddVaccineType", this.addPosts)
          .then((res) => {
            if (res.flag) {
              this.$message.success("添加疫苗种类信息成功");
              this.addFormVisible = false;
              this.selectPage(); // 重新查询数据
            } else {
              this.$message.error("添加疫苗种类信息失败");
              this.addFormVisible = true;
            }
          })
          .catch((error) => {
            this.$message.error("添加疫苗种类信息出错：" + error.message);
          });
      },
    //查询函数
    selectPage() {
